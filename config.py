...
VERSION_NOTES = "Версия 0.13.10. Фиксы по фидбеку тестеров: 1) снова применяются расходники в бою подземелья (настойка, морс и др.) — в кнопке слота callback парсился как enemy_id и давал ValueError; 2) закрыта гонка заброса удочки — лок ставится сразу до первого await, двойной тап больше не позволяет забрасывать многократно подряд и не списывает ОД/наживку дважды; 3) в описании подземного водохранилища убраны конкретные виды рыб — нейтральная фраза о редком улове. Ранее: v0.13.9 логирование и разбор проблем по игрокам."

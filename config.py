...
VERSION_NOTES = "Версия 0.11.16. Покупка в магазине: результат «Куплено» теперь показывается прямо в карточке товара (in place) вместо отдельного сообщения — не теряется клиентом; карточка обновляется с актуальным остатком и числом в инвентаре. Ранее: v0.11.15 хотфикс продажи; v0.11.14 подтверждение и выбор количества при продаже; v0.11.13 «Одежда»."

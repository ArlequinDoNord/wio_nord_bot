...
VERSION_NOTES = "ГосСМИ: новое здание в городе и вкладка «Новости Нордхайма» в главном меню — лента последних 10 выпусков. Публикуют журналист (до 2/сутки) и редактор (до 4/сутки), редактор/суперадмин редактируют и удаляют выпуски. Новость — заголовок, текст и опциональное фото. Полный архив выпусков — в Библиотеке."
...
# Пиво: напиток, который пьётся где угодно (в бою подземелья дополнительно даёт +HP).
# Дневной лимит: 3-я бутылка за сутки → «пьян»; 6-я (ещё 3 после «пьян») → «очень пьян»
# (заменяет «пьян», снимается только через 6 часов).
BEER_ITEM_NAME = "Бутылка пива"
BEER_DAILY_LIMIT = 3
BEER_VERY_DRUNK_LIMIT = 6
BEER_DRUNK_MINUTES = 60
BEER_VERY_DRUNK_MINUTES = 360       # 6 часов

...
VERSION_NOTES = "Версия 0.13.12. Подземелье больше не «залипает»: выход в город завершает активный забег (лут и НМ переносятся в инвентарь), забег дольше 1 часа сбрасывается сам — рыбалка и другие зоны больше не блокируются «⛔ Ты в подземелье». Ранее: v0.13.11 починены кнопки редактирования наград."

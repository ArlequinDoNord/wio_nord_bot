...
VERSION_NOTES = "Версия 0.13.5. Жильё: картинки стадий растений в кадке — под «Семя» общая кадка, дальше (Росток→Саженец→Цветущее→Плодоносящее) картинки под конкретное дерево (вместе с переименованием стадий). Ранее: v0.13.4 подтверждение выхода из подземелья и лок рыбалки."

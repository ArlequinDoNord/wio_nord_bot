...
VERSION_NOTES = "Сырая рыба хранится 4 дня, продаётся на рынок (Расходники) с заморозкой срока; в Ратуше карточка пилота минимальна; архив СМИ и Библиотеки — по читательскому билету; админ-меню больше не пропадает; кухня — через кнопку «Готовить»; кадка с растением требует подтверждения; жильё показывается в предпросмотре магазина. Хотфикс: исправлен AttributeError в каталоге магазина для sqlite3.Row."

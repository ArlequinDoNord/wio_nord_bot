...
def escape_chance(player_hp_percent: float, dice_roll: int = None, smoke_used: bool = False,
                  percent_mult: float = 1.0) -> bool:
    """
    ...
    percent_mult — множитель шанса (напр. 0.5 для «очень пьян»), применяется
    к итоговому проценту ДО перевода в d20.
    """
    if dice_roll is None:
        dice_roll = roll_dice(20)
    if smoke_used:
        percent = DUNGEON_SMOKE_ESCAPE * percent_mult
        success = max(1, min(20, percent * 20 // 100))
        return dice_roll <= success
    base_percent = ESCAPE_BASE_CHANCE + int((1.0 - player_hp_percent) * ESCAPE_HP_BONUS)
    percent = base_percent * percent_mult
    success = max(1, min(20, percent * 20 // 100))
    return dice_roll <= success
